...
void loop(){  
  if(IS_BASIC == 1){ //It's in basic mode 
    ...
    if(irrecv.decode(&keycode)){
     //will scan for a key press 
     uint16_t key_pressed = (keycode.value & 0xFFFF); 
     
      if(key_pressed == KEY_OK){ //this causes some issues, but it still works --------------------------------------------------------------
        if(IS_BASIC == 1){
          IS_BASIC = 0;
    ...
    //do extra shit
    if(irrecv.decode(&keycode)){ //will scan for a key press 
      uint16_t key_pressed = (keycode.value & 0xFFFF); // just following the stuff from before
...
void processInstruction( uint16_t key){
  switch (key) {
    ...
    case KEY_OK: IS_BASIC = 1; break; //change the value of IS_BASIC back to true
