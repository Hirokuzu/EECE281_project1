...
void loop(){  
  ...
  if(ir_recv.decode(&keycode)){
    ...
    irFlag = 1;
    Serial.print("in decode");
  }
  ir_recv.resume();
  
  if(IS_BASIC == 1){ //It's in basic mode 
    Serial.print(" in basic ");
    operate_in_basic_mode(); //So it should do basic functionality that was tested
     
    if(key_pressed == KEY_OK && irFlag == 1){ //this hasn't been tested for working --------------------------------------------------------------
  ...
  } else if (IS_BASIC == 0 && irFlag == 1){ //will scan for a key press and in extra mode
    Serial.print(" in extra ");
